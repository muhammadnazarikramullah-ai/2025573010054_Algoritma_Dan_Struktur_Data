...
// 5. Ambil nama produk dari yang tersedia
const namaProduk = produkTersedia.map((item) => item.nama);
console.log("Nama produk tersedia:", namaProduk);

// 6. Hitung total nilai inventaris (harga * stok)
const totalInventaris = produkTersedia.reduce((total, item) => {
  return total + item.harga * item.stok;
}, 0);

console.log("Total nilai inventaris: Rp" + totalInventaris.toLocaleString());

// 7. Tampilkan semua produk dengan format tertentu
console.log("\nDaftar Produk:");
produk.forEach((item) => {
  if (item.stok > 0) {
    console.log(
